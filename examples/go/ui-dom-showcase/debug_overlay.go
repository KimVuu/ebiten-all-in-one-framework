...
func buildDebugLayoutReport(layout *uidom.LayoutNode, viewport uidom.Viewport) debugLayoutReport {
	...
	validation := uidom.ValidateLayout(layout, viewport, uidom.ValidationOptions{})
	for _, issue := range validation.Issues {
		...
		case uidom.IssueSeverityError:
			report.SummarySnapshot.Errors++
		case uidom.IssueSeverityWarning:
...
func buildDebugUISnapshot(layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, overlayEnabled bool, runtime *uidom.Runtime, input uidom.InputSnapshot, queueDepth int) ebitendebug.UISnapshot {
	if layout == nil {
...
func convertLayoutToUISnapshot(layout *uidom.LayoutNode, parent *uidom.LayoutNode, report debugLayoutReport, viewport uidom.Viewport, overlayEnabled bool) ebitendebug.UINodeSnapshot {
	...
	if layout.Node.Tag == uidom.TagTextBlock && len(layout.TextLines) > 0 {
...
func semanticMetadata(layout *uidom.LayoutNode) *ebitendebug.UISemanticSnapshot {
	if layout == nil || layout.Node == nil {
		return nil
	}
	spec := layout.Node.Props.Semantic
	if spec.Screen == "" {
		spec.Screen = "ui-dom-showcase"
	}
	if spec.Element == "" {
...
func interactionStateMetadata(layout *uidom.LayoutNode) map[string]any {
	if layout == nil || layout.Node == nil {
		return nil
	}
...
func nodeRole(layout *uidom.LayoutNode) string {
	if layout == nil || layout.Node == nil {
		return ""
	}
	switch layout.Node.Tag {
	case uidom.TagHeader:
		return "header"
	case uidom.TagMain:
		return "main"
	case uidom.TagSection:
		return "section"
	case uidom.TagFooter:
		return "footer"
	case uidom.TagButton:
		return "button"
	case uidom.TagScrollView:
		return "scroll"
	case uidom.TagImage:
		return "image"
	case uidom.TagText, uidom.TagTextBlock:
		return "text"
	case uidom.TagSpacer:
		return "spacer"
	case uidom.TagStack:
		return "stack"
	default:
...
func nodeSlot(layout *uidom.LayoutNode) string {
	if layout == nil || layout.Node == nil {
		return ""
	}
	id := layout.Node.Props.ID
...
func layoutMetadata(layout *uidom.LayoutNode) *ebitendebug.UILayoutSnapshot {
	if layout == nil || layout.Node == nil {
		return nil
	}
	...
	if result.Anchor == "" {
		result.Anchor = ebitendebug.UIAnchor(uidom.AnchorTopLeft)
	}
	if result.Pivot == "" {
		result.Pivot = ebitendebug.UIPivot(uidom.PivotTopLeft)
	}
	if spec.MinSize != (uidom.LayoutSize{}) {
	...
	if spec.MaxSize != (uidom.LayoutSize{}) {
...
func computedMetadata(layout *uidom.LayoutNode, parent *uidom.LayoutNode) *ebitendebug.UIComputedSnapshot {
	if layout == nil {
		return nil
	}
...
func buildUIInputState(runtime *uidom.Runtime, input uidom.InputSnapshot, queueDepth int) ebitendebug.UIInputSnapshot {
...
func rectToDebug(rect uidom.Rect) ebitendebug.Rect {
...
func derivedLayoutMode(node *uidom.Node) uidom.LayoutMode {
	if node == nil {
		return uidom.LayoutModeFlowVertical
	}
	if node.Props.Layout.Mode != "" {
		return node.Props.Layout.Mode
	}
	switch {
	case node.Tag == uidom.TagStack:
		return uidom.LayoutModeStack
	case node.Tag == uidom.TagScrollView:
		return uidom.LayoutModeFlowVertical
	case node.Props.Style.Direction == uidom.Row:
		return uidom.LayoutModeFlowHorizontal
	default:
		return uidom.LayoutModeFlowVertical
	}
}

func resolvedLayoutLength(length uidom.Length, fallback float64) float64 {
	switch length.Kind {
	case uidom.LengthPx:
		return length.Value
	case uidom.LengthFill:
		return fallback
	default:
		return fallback
	}
...
func drawDebugOverlay(screen *ebiten.Image, layout *uidom.LayoutNode, report debugLayoutReport, overlayEnabled bool) {
...
func drawDebugOverlayNode(screen *ebiten.Image, layout *uidom.LayoutNode, report debugLayoutReport) {
	if layout == nil || layout.Node == nil {
		return
	}
...
func outOfViewport(frame uidom.Rect, viewport uidom.Viewport) bool {
...
func outOfParent(frame, parent uidom.Rect) bool {
...
func outsideSafeArea(frame uidom.Rect, viewport uidom.Viewport, inset float64) bool {
	if viewport.Width <= 0 || viewport.Height <= 0 {
		return false
	}
	left := inset
...
func textOverflows(layout *uidom.LayoutNode) bool {
	if layout == nil || layout.Node == nil {
		return false
	}
...
func intersects(a, b uidom.Rect) bool {
