...
type compactNodeRef struct {
	layout *uidom.LayoutNode
	parent *uidom.LayoutNode
}

func buildCompactUIOverview(layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, runtime *uidom.Runtime, input uidom.InputSnapshot, queueDepth int) ebitendebug.UIOverviewSnapshot {
...
func queryCompactUINodes(layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, request ebitendebug.UIQueryRequest) ebitendebug.UIQueryResult {
	refs := flattenCompactLayout(layout)
...
func inspectCompactUINode(layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, request ebitendebug.UINodeInspectRequest) (ebitendebug.UINodeDetailSnapshot, bool) {
...
func captureCompactUIScreenshot(gameID string, layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, request ebitendebug.UICaptureRequest) (ebitendebug.UICaptureResult, ebitendebug.UIArtifact, bool) {
	if layout == nil {
...
func flattenCompactLayout(root *uidom.LayoutNode) []compactNodeRef {
	if root == nil {
		return nil
	}
	result := []compactNodeRef{}
	var walk func(node *uidom.LayoutNode, parent *uidom.LayoutNode)
	walk = func(node *uidom.LayoutNode, parent *uidom.LayoutNode) {
...
func matchesCompactQuery(ref compactNodeRef, viewport uidom.Viewport, report debugLayoutReport, request ebitendebug.UIQueryRequest) bool {
...
func compactSummaryForNode(layout *uidom.LayoutNode, parent *uidom.LayoutNode, report debugLayoutReport, viewport uidom.Viewport) ebitendebug.UINodeSummarySnapshot {
	if layout == nil || layout.Node == nil {
...
func compactPropsForNode(layout *uidom.LayoutNode) map[string]any {
	if layout == nil || layout.Node == nil {
		return nil
	}
...
func collectDirectChildSummaries(layout *uidom.LayoutNode, report debugLayoutReport, viewport uidom.Viewport) []ebitendebug.UINodeSummarySnapshot {
	if layout == nil {
		return nil
	}
...
func inViewport(rect uidom.Rect, viewport uidom.Viewport) bool {
	view := uidom.Rect{X: 0, Y: 0, Width: viewport.Width, Height: viewport.Height}
...
func renderLayoutToRGBA(layout *uidom.LayoutNode, viewport uidom.Viewport, report debugLayoutReport, withOverlay bool) *image.RGBA {
...
func drawCompactNode(dst *image.RGBA, layout *uidom.LayoutNode) {
	if layout == nil || layout.Node == nil {
		return
	}
	frame := image.Rect(
...
func drawCompactInteractionState(dst *image.RGBA, frame image.Rectangle, layout *uidom.LayoutNode) {
...
func drawCompactOverlay(dst *image.RGBA, layout *uidom.LayoutNode, report debugLayoutReport) {
...
func resolveCaptureRect(layout *uidom.LayoutNode, viewport uidom.Viewport, request ebitendebug.UICaptureRequest) (image.Rectangle, bool) {
	...
	case "node_id":
		...
		rect := target.Frame
		if target.Node != nil {
			if target.Node.ClickableRect != (uidom.Rect{}) {
...
func resolvePointerTargetAt(layout *uidom.LayoutNode, x, y float64) (debugResolvedTarget, bool) {
	refs := flattenCompactLayout(layout)
