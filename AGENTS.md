...
모든 구현 작업은 같은 순서를 따른다. 먼저 현재 테스트와 검증 명령을 확인하고, 필요한 테스트를 먼저 추가한 뒤 구현을 진행한다. 구현이 끝나면 테스트를 다시 돌리고, 관련 문서를 갱신한 뒤 기본적으로 로컬 `git commit`까지 완료한다. `push`와 원격 작업은 별도 요청이 있을 때만 진행한다.
