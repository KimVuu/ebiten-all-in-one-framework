...
`libs/go/ui-dom`은 Ebiten에서 선언형으로 UI 트리를 만들기 위한 Go 라이브러리다. HTML 태그처럼 `Div`, `Section`, `Text` 같은 노드를 조합하고, React처럼 함수가 노드를 반환하는 방식으로 화면 구조를 만들 수 있게 두는 것을 목표로 한다.
	...
	return uidom.New(
		uidom.Div(uidom.Props{
			...
			uidom.Header(uidom.Props{},
				uidom.Text("Ebiten UI DOM", uidom.Props{
					ID: "title",
					Style: uidom.Style{
			...
			uidom.Section(uidom.Props{
				...
				uidom.Text("Build UI like a DOM tree.", uidom.Props{
					Style: uidom.Style{
...
렌더링할 때는 `ebitenrenderer.Renderer`로 `*ebiten.Image`에 그리면 된다.

## 런타임 계층

`ui-dom`은 이제 정적 노드 트리만이 아니라 입력 런타임도 제공한다.
...
layout := runtime.Update(dom, uidom.Viewport{
	Width:  1280,
	Height: 720,
}, uidom.InputSnapshot{
	PointerX:    mouseX,
...
- `uidom`: 노드, 스타일, 레이아웃, DOM 조회
- `uidom/ebitenrenderer`: Ebiten 이미지 렌더링
- `uidom/prefabs`: 게임 UI 프리셋

## 예제

- 통합 쇼케이스: [examples/go/ui-dom-showcase/README.md](../../../examples/go/ui-dom-showcase/README.md)
